...
// list라는 경로로 접속하면 해당 list.html 파일을 보내준다.

// 페이지 나누는 법(리액트 사용시)
// SPA라서 index.html 파일 하나만 존재
// List 컴포넌트를 만듦
// /list라는 경로로 접속하면 기존 컴포넌트 자리에 List라는 컴포넌트를 보여준다.
...
// 사용자가 주소창에 새로운 경로를 입력하거나 페이지 내에서 경로이동이 발생되면
  ...
  return (
    <BrowserRouter>
      <Routes>
        ...
        {/* Nested route 방식 */}
        {/* 위 서브 경로 방식과 차이점: 부모 엘리먼트 + 자식 엘리먼트가 보인다. */}
        {/* 부모 안에 자식들을 렌더링해 보여줌(어디에 보여줄지는 부모안에서 Outlet 컴포넌트로 지정) */}
        {/* Nseted 안에 또 Nested 중복 가능 */}
        <Route path='/games' element={<GamePage />}>
