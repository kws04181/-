...
function App() {
  ...
  // todos 배열에 새 객체를 생성하기 위한 handleInsert() 함수 정의
  // 새로운 객체를 생성할 때마다 id값에 1씩 더해 주어야 하는데 useRef()를 사용하여 변수 생성
  // id값은 렌더링되는 정보가 아니기 때문에 ref 사용
  // 단순히 새로운 항목을 만들 때 참조되는 값임

  const nextId = useRef(4);

  // props로 전달해야 할 함수를 만들 때는 useCallback()을 사용해보기
  ...
  const handleRemove = useCallback((id) => {
    ...
    // 방법2 - 배열의 내장 함수 이용
    // filter('테스트 함수'): 기존의 배열은 변경하지 않고 특정 조건을 만족하는 요소들만 따로 추출하여
    // 새로운 배열을 만든다. 

    // 테스트 함수에서는 true 또는 false를 반환해야 하며,
...
// HTML 웹 스토리지란?
// 웹 스토리지를 사용하면 웹 앱이 사용자의 브라우저 내에 로컬로 데이터를 저장할 수 있다.
// 웹 스토리지는 도메인 당 사용 가능
// 같은 도메인의 모든 페이지는 동일한 데이터를 저장하고 액세스 한다.

// 웹 스토리지 객체
// 웹 스토리지는 데이터를 저장하기 위한 두 가지 객체를 제공
// window.localStorage - 만료 날짜 없이 데이터를 저장
// window.sessionStorage - 한 세션에 대한 데이터 저장(브라우저 탭을 닫으면 데이터 손실)
